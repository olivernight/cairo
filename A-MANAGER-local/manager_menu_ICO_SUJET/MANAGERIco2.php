<?php

$REP = $_POST['REP'];
$cible = $_POST['cible'];

include("../Controle/Sauv/sauv.php");
$array=array("16");
sauv($file,$array);

$n=$_REQUEST["N"];
$nb_valeurs=count($n);

//on ouvre le fichier
$fichier='../../'.$REP.'/'.$cible;
if($cible=="DATA-IcoSujet.js"){$sufix="IconeEchelle";$saisie="ICO-sujet";$index="2000";};
if($cible=="DATA-IcoOther.js"){$sufix="IconeSup";$saisie="ICO-other";$index="1000";};
if($cible=="DATA-Sujet.js"){$sufix="Sujet";$saisie="GRAPH-sujet";$index="0";};
if($cible=="DATA-Other.js"){$sufix="Other";$saisie="GRAPH-other";$index="100";};
$op_file=fopen($fichier,"w+");
//on ecrit dans le fichier : 
$j=0;
$z=0;
$a=0;
//for($i=0;$i<$nb_valeurs;$i++){$n[$i] = htmlspecialchars($n[$i]);}
for($i=0;$i<$nb_valeurs;$i++){
//$n[$i] = utf8_decode($n[$i]);
//$n[$i] = utf8_encode($n[$i]);
}

for($i=0;$i<$nb_valeurs;$i++)
{

if($j==0 and $n[$i]=="" and $n[$i+1]==""  and $n[$i+2]=="" ){$a=1;$i=$i+13;}else{$a=0;}
if($a==0){
//$n[$i]=str_replace("<","&lt;",$n[$i]);
//$n[$i]=str_replace(">","&gt;",$n[$i]);
if($j==0 ){
fwrite($op_file,"// ---------------------------MENU n°".($i/13+1)."----------------------------------------------\n");
fwrite($op_file,"paramHISTOX0=new Array()\n");
};
if($j==0){fwrite($op_file,"paramHISTOX0[".$j."]=new Array(".$n[$i].")\n");}
if($j==3){fwrite($op_file,"paramHISTOX0[".$j."]=new Array(".$n[$i].")\n");$codegraph=$n[$i];};

if($j==1 or $j==2 or $j==9 or $j==12 or $j==6  or $j==7 ){fwrite($op_file,"paramHISTOX0[".$j."]=\"".$n[$i]."\"\n");};
if($j==4){ 
$n[$i] = StripSlashes($n[$i]); 
$n[$i] = str_replace ( "x@x" , "\'" , $n[$i] ); 

fwrite($op_file,"paramHISTOX0[".$j."]=new Array(".$n[$i].")\n");};
if($j==8){
if($n[$i-5]=="3,1"){
fwrite($op_file,"paramHISTOX0[".$j."]=new Array(\"".$n[$i]."\")\n");
}else{
fwrite($op_file,"paramHISTOX0[".$j."]=new Array(\"\",\"\")\n");
};

};//new Array(\"\",\"\")

if($j==5 or $j==11 ){
if($j==5 & $codegraph=="1,2"){$n[$i]="";} 


$n[$i] = StripSlashes($n[$i]); 
$n[$i] = str_replace ( "x@x" , "'" , $n[$i] ); 


fwrite($op_file,"paramHISTOX0[".$j."]=new Array(".$n[$i].")\n");};

if($j==10 ){fwrite($op_file,"paramHISTOX0[".$j."]=".$index."+menu".$sufix.".length+1\n");};
if($j==12){fwrite($op_file,"menu".$sufix."[menu".$sufix.".length]=paramHISTOX0\n\n");};
//echo "<br>n[".$i."] = ".$n[$i]."  --> inscrit dans ".$fichier." ! ";

$j=$j+1;
if($j==13){$j=0;}
}
}
echo "<br><b>Modifications enregistrées dans ".$fichier." </b>";
echo '<script language="javascript">parent.frames.framefichiers.document.location.href="Saisie'.$saisie.'.php?REP='.$REP.'&cible='.$cible.'&alea="+Math.random()</script>';
fclose($op_file);
?>