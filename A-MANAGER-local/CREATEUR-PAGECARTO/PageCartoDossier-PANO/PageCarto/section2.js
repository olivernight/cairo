...
/*
for(k=0;k<inhibaireG.length;k++){
alert(k)
if((iSujet+1)==inhibaireG[k][0]){
alert("ici")
var vecteuraire=inhibaireG[k]
for(h=1;h<vecteuraire.length;h++){
xdata=-99999
}
}
}
*/					
					// inhibition  via les données du fichier inHibAire.js
					for(k=0;k<inhibaireG.length;k++){
					var vecteuraire=inhibaireG[k]
					
					for(h=1;h<vecteuraire.length;h++){
					if((iSujet+1)==inhibaireG[k][0] & NoDatx==vecteuraire[h]){
					//alert(h)
					xtab=-99999
					}
					}
...
function calculIcone(Icone){ // a est le rang dans les aires indexées du SVG et b est le rang dans le tableau menuIconeEchelle
...
basex[i]=parseInt(100*100*num/denom)/100
...
var ResX=parseInt(100*SnumX/SdenomX)/100;ResX=parseInt(10000*ResX)/100
var perc="%"
...
if(num1==-99999 || num2==-99999 || denom==-99999 || denom==0){basex[i]=-99999}else{
			...
			//}
basex[i]=parseInt(100*100*(num1-num2)/denom)/100
...
if(SdenomX==100){
var perc="%"
var ResX=parseInt(100*Synthesetotal/SdenomX)/100
...
SynthX+="</tr><tr><td><b><span style='color:black'>valeur de l'origine : a = </span>"+SdepX+"</b></td></tr>"
...
SynthX+="<tr><td><b><span style='color:black'>Dénominateur : c= </span>"+SdenomX+"</b></td></tr>"
var ResX=Synthesetotal/SdenomX;ResX=parseInt(10000*ResX)/100
var perc="%"
if(menuIconeEchelle[Icone][3][8]){if(menuIconeEchelle[Icone][3][8]!=-99999){ResX=ResX*menuIconeEchelle[Icone][3][8]/100;perc=""}}
...
var perc="%"
var ResX=SnumX/SdenomX;ResX=parseInt(10000*ResX)/100
if(menuIconeEchelle[Icone][3][8]){if(menuIconeEchelle[Icone][3][8]!=-99999){ResX=ResX*menuIconeEchelle[Icone][3][8]/100;perc=""}}
...
for(i=1;i<inhibaire.length;i++){
if((Icone+1)==inhibaire[i][0]){
var vecteuraire=inhibaire[i]
for(j=1;j<vecteuraire.length;j++){
basex[vecteuraire[j]]=-99999
}
}
}
