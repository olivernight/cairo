...
var largeurHistoGRAND=270//355//350
var Histomulti=0
var largeurHisto=200
var GrandHisto=0
var HISTOLA=''

var agred=agG//"Agrandir le graphique"
...
if(a.innerHTML==agG){//"Agrandir le graphique"

if(clickOuHyper==0){GrandHisto=1}
recupDim()//RADAR
agred=redG//"Réduire le grahique"
...
document.getElementById("innerhisto").style.left=(leftHisto-largeurHistoGRAND+200)+"px"
		...
		document.getElementById("innerhisto").style.left=(leftHisto-largeurHistoGRAND+Xdim[0]-(largeurHistoGRAND-largeurHisto)+3)+"px"
		document.getElementById("innerhisto").style.top=(topHisto-(largeurHistoGRAND-largeurHisto)-3)+"px"
		}
...
document.getElementById("innerhisto").style.left=leftHisto+"px"
		...
		if((menuSujet[iSujet][3][0]==1 & menuSujet[iSujet][3][1]==3)){//RADAR
		document.getElementById("innerhisto").style.left=(leftHisto-largeurHistoGRAND-200+Xdim[0]+3)+"px"
...
libelhistomultiout(0)
				...
				//-----------------------------------------------------------------------------------------------------------------------
							...
							Xdim[0]=largeurHistoGRAND // dimension du carre d'encadrement
...
function returnRadarOuCourbe(){
var RadarOuCourbe=new Array(LaCourbe,Radar)	
LaCourbe=0
Radar=0	
return RadarOuCourbe	
}							
var LaCourbe=0
var Radar=0
var sautl
...
if(GrandHisto==1 ){largeurHisto=largeurHistoGRAND; largcol=largcol2*largeurHistoGRAND/200;DimH=DimH2*largeurHistoGRAND/200*2/3}else{ largcol=largcol2; DimH=DimH2}
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
histola+='<table  cellpadding="5" width="'+largeurHisto+'px" cellspacing="0" style="border:0.0px solid gray;font-family:arial;background-color:white;"><tr><td>'
...
var poltit0=12
var heightla=15
//if(titreX[0].length>15){poltit0=10 }else{}
if(titreX[0].length>23){poltit0=10;}
if(titreX[0].length<=24){heightla=25 }else{heightla=35;poltit0=10}
histola+='<td style="color:navy;height:'+heightla+'px;vertical-align:top"><div  style="position:absolute"><div style="font-size:'+poltit0+'px;opacity:0.6;position:relative;top:-20px"><big><big><b>'+titreX[0].replace(/-/g,' ').replace(/_/g,' ')+'</b></big></big></div></div></td>'


if(GrandHisto==0){var largtitre=130;}else{var largtitre=largeurHisto;}
var poltit="10"
var heightla=15
var topla=-25
if(titreX[1].length>36){
/*poltit=9*/ ;
titreX[1]=titreX[1].replace("<br/>"," ")
heightla=30	
topla=-35
}
if(titreX[1].length>72){
/*poltit=9*/ ;
titreX[1]=titreX[1].replace("<br/>"," ")
heightla=30	
topla=-32
}
histola+='</tr><tr height="15px" align="left"><td></td><td style="font-size:'+poltit+'px;height:'+heightla+'px;"><div  style="position:absolute"><div style="opacity:0.6;position:relative;top:'+topla+'px;width:'+(largtitre-20)+'px"><big><b>'+titreX[1]+'</b></big></div></div></td></tr><tr><td style="width:10px"></td><td>'//(iSujet+1)+' - '+
...
//--------------------------------------------------------------------------------fin de NEGATIFSonmouseout="libelhistomultiout(this)" onmouseover="libelhistomultiover(this)"
						...
						histola+='<tr><td><div><div style="position:absolute;right:100px"><div style="position:relative;top:-0px" id="aflibel"> </div></div></div></td></tr>'
						histola+='</table>'
...
histola+='</tr></table>'
histola+='</td><td style="width:3px"></td><td style="font-size:8px" title="'+libel[i]+'"><table border="0" cellpadding="0" cellspacing="0" width="100px"><tr ><td>'+libel[i].substring(0,25)+'</td></tr><tr style="height:1px"><td style="background-color:white;width:100%"></td></tr></table></td><td style="width:2px"></td></tr>'
...
// RADAR
Radar=0
if(menuSujet[iSujet][3][0]==1 & menuSujet[iSujet][3][1]==3){
	//alert("Radar section3 essaiBLANC = "+essaiBLANC)
	
	if(essaiBLANC==1){
	HISTOLA='<div style="position: absolute;top:-20px;left:5px"><div style="position:relative;left:-5px"><table style="background-color:white;border : solid black 0px;-moz-border-radius: 5px 5px 0px 0px;-moz-border-top-right-radius: 5px;"><tr><td><a style="opacity:0.5" id="commandeTialleHisto" href="javascript:EtatHisto()">'+agred+'</a></td></tr></table></div></div><iframe id="zam2" name="zam2" width="200px" height="200px" scrolling="no" frameborder="0" src="../PageCartoDossier-BLANC/PageCarto/GRAPH-RADAR/radar.xml" ></iframe>';	
	Radar=1
	}else{
...
histola+='<table  cellpadding="5" width="'+largeurHisto+'px" cellspacing="0" style="border:0px solid gray;font-family:arial;background-color:white;"><tr><td>'
histola+='<table border="0" cellpadding="0" cellspacing="0"><tr><td>'
...
histola+='</tr><tr height="15px" align="left"><td></td><td style="font-size:'+poltit+'px;"><div  style="position:absolute"><div style="position:relative;top:-15px"><b>'+(iSujet+1)+' - '+titreX[1]+'</b></div></div></td></tr><tr><td style="width:10px"></td><td>'
						histola+='<table border="0" cellpadding="0" cellspacing="0">'
						histola+='<tr height="5px"><td></td></tr><tr  style="height:'+(DimH*0.5+0)+'px" >'
//-------------------------------------------------------------POSITIFS
var fsize=8
...
if(parseFloat(menuSujet[iSujet][7])>10){
var libici='<span style="color:red">'+libiciX+'</span><br/>'+menuSujet[iSujet][4][(i-1)/parseFloat(menuSujet[iSujet][6])].substring(0,10);
}else{
var libici='<span style="color:red">'+libiciX+'</span><br/>'+menuSujet[iSujet][4][(i-1)/parseFloat(menuSujet[iSujet][6])].substring(0,20);
}
bord=0.1; }
if(Hhi[i+j-1]==0){bord=0}
histola+='<tr style="height:'+(Hhi[i+j-1]*0.9)+'px" title="'+libiciX+'-'+menuSujet[iSujet][5][j-1]+' '+menuSujet[iSujet][4][(i-1)/parseFloat(menuSujet[iSujet][6])]+' : '+TAB[i+j-1]+' '+pct+'"  onmouseout="libelhistomultiout(this)" onmouseover="libelhistomultiover2(this)" >'
...
//histola+='<tr align="center"><td ><div style="position:absolute;font-size:5px" onmouseout="libelhistomultiout(this)" onmouseover="libelhistomultiover(this)">'+libici+'</div></td></tr>'((largeurHisto/Hhi.length)-(2*1+larginter)*(i+1))<span style="color:transparent">-</span>   (sautl+libici)<div style="position:absolute;width:'+(largeurHisto/parseFloat(menuSujet[iSujet][7]))/2+'px;" ></div>

//histola+='<tr ><td style="border:1px solid red"><div style="position:absolute;"><div style="font-size:'+fsici+'px;position:relative;left:-'+(largeurHisto/parseFloat(menuSujet[iSujet][7]))/2*0+'px" ><table width="'+(largeurHisto/parseFloat(menuSujet[iSujet][7]))+'px"><tr><td><center title="'+menuSujet[iSujet][4][(i-1)/parseFloat(menuSujet[iSujet][6])]+'">'+(sautl+libici)+'</center></td></tr></table></div></div></td><td width="'+larginter+'"></td></tr>'
histola+='<tr ><td style="border:0px solid red"><div style="position:absolute;"><div style="font-size:'+fsici+'px;position:relative;left:-'+(largeurHisto/parseFloat(menuSujet[iSujet][7])/4)+'px; width:'+(largeurHisto/parseFloat(menuSujet[iSujet][7]))+'px"><center title="'+menuSujet[iSujet][4][(i-1)/parseFloat(menuSujet[iSujet][6])]+'">'+(sautl+libici)+'</center></div></div></td><td width="'+larginter+'"></td></tr>'
...
if(essaiBLANC==1){
if(menuSujet[iSujet][3][0]==3 & menuSujet[iSujet][3][1]==1){;HISTOLA='<div style="position: absolute;top:-20px;left:5px"><div style="position:relative;left:-5px"><table style="background-color:white;border : solid black 0px;-moz-border-radius: 5px 5px 0px 0px;-moz-border-top-right-radius: 5px;"><tr><td><a style="opacity:0.5" id="commandeTialleHisto" href="javascript:EtatHisto()">'+agred+'</a></td></tr></table></div></div><iframe id="zam2" name="zam2" width="200px" height="200px" scrolling="no" frameborder="0" src="../PageCartoDossier-BLANC/PageCarto/GRAPH-COURBE/courbe.xml"></iframe>';histola=HISTOLA
LaCourbe=1
