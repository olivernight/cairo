 ...
 */
echo'<script language="javascript" src="../MAJlog.js?updated=1234567890"></script> ';
...
// FIN CONFIGURATION
		...
		$filter = array( 'lister','A0-ModuleMAJ','Controle','images','lister','telechargements','MAJ','A0-ModuleMAJ.zip');
		$filterB = array( '','');
...
$inhib1=0; // 0 pour activer le listage des répertoires
			...
			for ( $i = $inhib1; $i < count( $contents ); $i++ ) {
				...
				$a="1".$fileinfo[1];
		if(strpos($a,"A0-ModuleMAJ")>=1 & $fileinfo[1]!="A0-ModuleMAJ"){
				...
				if( MAJ[i].indexOf(fich)>0){compar=0;i=MAJ.length}
				}
				if(compar==1){
				listefile[listefile.length]=new Array(\''.$fileinfo[1].'\',\''.$fileinfo[2].'\',\''.$fileinfo[3].'\');
				ecritfile+="<br><span  style=\"position:relative;left:-50px;font-family:arial;font-size:12px;color:blue;opacity:0.7\"><b> <a href=\"../'.$fileinfo[1].'/lister/ModuleMAJ.php\" target=\"_self\">'.$fileinfo[1].'</a></b></span>"
