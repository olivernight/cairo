	...
	************************************************************************************************************/	
	...
	var dragableElementXMoved = new Array();
	...
	*/	
	...
	function initDragDropElement(e)
	{
		...
		if(!dragableElementXMoved[numericIdToBeDragged]){
	...
	function timerDragDropElement()
	{
		...
		if(dragDropTimer>=10){
			if(dragObjCloneArray[numericIdToBeDragged].style.display=='none'){
				...
				dragObjCloneArray[numericIdToBeDragged].style.top = getTopPos(dragObjArray[numericIdToBeDragged]) + 'px';
				dragObjCloneArray[numericIdToBeDragged].style.left = getLeftPos(dragObjArray[numericIdToBeDragged]) + 'px';
				dragableElementXMoved[numericIdToBeDragged] = true;	
			}
		}		
	}
	...
	function movedragableElementX(e)
	{
		if(document.all)e = event;		
		...
		if(dragObjCloneArray[numericIdToBeDragged].id=='box2'){
		document.getElementById("box3").style.top=(parseInt(dragObjCloneArray[numericIdToBeDragged].style.top)+20)+'px'
		document.getElementById("box3").style.left=dragObjCloneArray[numericIdToBeDragged].style.left
		}
	}
	...
	function stop_dragDropElement()
	{
		...
		if(rememberPositionedInCookie && dragObjCloneArray[numericIdToBeDragged]){		
			dragableContent_cookieItems['dragableElementXClone' + numericIdToBeDragged] = [dragObjCloneArray[numericIdToBeDragged].style.left,dragObjCloneArray[numericIdToBeDragged].style.top,dragObjCloneArray[numericIdToBeDragged].style.zIndex]; 
		}
	...
	function initdragableElementXs()
	{
		...
		for(var no=0;no<allObjects.length;no++){
			if(allObjects[no].className=='dragableElementX'){
		...
		for(var no=0;no<tmpElements.length;no++){
			...
			tmpElements[no].className='dragableElementX' + no;
			el.onmousedown = initDragDropElement;
			el.className='dragableElementXClone' + no;			
		...
		document.body.onmousemove = movedragableElementX;
	...
	function positionItemsFromCookie()
	{
		for(var prop in dragableContent_cookieItems){
			for(var no=0;no<dragObjCloneArray.length;no++){
				...
				//document.getElementById('cadrevideo').style.visibility='hidden'
					...
					currentZIndex = Math.max(currentZIndex,dragableContent_cookieItems[prop][2]/1 + 1);
								...
								document.getElementById("box3").style.top=(parseInt(document.getElementById("box2").style.top)+20)+'px'
document.getElementById("box3").style.left=document.getElementById("box2").style.left
	}
	
	window.onload = initdragableElementXs;
