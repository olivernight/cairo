<?php 
					...
					if ( is_file( $dir . $filename ) ) {
						...
						if ( strpos( $filename, "." ) !== 0 ) {
							$ck=true;
								...
								if($ck==true)
									array_push( $result, array( "folder", $filename, format_byte_string( filesize( $dir . $filename ) ), date( "y,m,d,g,i,A", filemtime( $dir . $filename ) ) ) );
						}
					}
					$filename = readdir( $dhandle );
				}
	...
	function file_properties ( $dir, $filename ) {
		...
		array_push( $result, date( "y,m,d,H,i,s", filemtime( $dir . $filename ) ) );
		//array_push( $result, date( "y,m,d,g,i,A", filemtime( $dir . $filename ) ) );
		return( $result );
	}
