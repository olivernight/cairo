...
tapx=tapx.indexOf("Site-",0)
//xxxxxxxxxxxxxxxxxxxxxx Correctif pour les sites datant d'avant la modification de l'affichage des graphiques à l'extérieur du cadre de la carte en novembre 2009

if(tapx>0){
if(top.document.getElementById("mobile2")){}else{
var ispan=top.document.createElement("span")
ispan.setAttribute("id","ispan")
top.document.getElementsByTagName("body")[0].appendChild(ispan)
top.document.getElementById("ispan").innerHTML='<div id="mobile2"    style="right:0px;top:10;visibility:visible;position:absolute;z-index:100000"><div id="bferm" style="position:absolute; visibility:hidden;top:5px;left:6px"><img id="ferm" src="../boutonferme.gif" onmouseover="javascript:top.frames.Num0.frames.couches.disable22();top.document.getElementById(\'zam2\').height=0;this.style.visibility=\'hidden\'" style="visibility:hidden"></div><iframe  ALLOWTRANSPARENCY="true" id="zam2" src="vide.htm" width=300 height=0 style="padding:0px;" FRAMEBORDER=0 scrolling="no" name="zam2"></iframe></div>'

}
var cdat=top.document.createTextNode("function forcevala(){var tapx=1;return tapx}")
var scri2=top.document.createElement("script")
top.document.getElementsByTagName("body")[0].appendChild(scri2)
...
//alert("cartonormal legiconeretrait="+legiconeretrait)

var mat="matrix(1 0 0 1 "+( xleg-xleg0)+" "+( yleg-yleg0+hautleg+legiconeretrait)+")"
//alert("reposeleg legiconeretrait="+legiconeretrait)
...
function even(){
if(document.allxxxx){ // gestion d'evꯥment IE
...
if(document.allxxxx){window.frames[2].zoompos()}
DOC.style.width=LL
DOC.style.height=HH
...
ajusttraitetpolice()
ajustetraiticone(1/(1+zoomFactor/100))
...
var VALEURRANGensemble
function inittraiticone(){	
for(i=1;i<VALEURRANGensemble;i++){
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("c"+i).setAttribute("stroke-width","1px")
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("n"+i).setAttribute("stroke-width","1px")
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("f"+i).setAttribute("stroke-width","1px")
}	
}

function ajustetraiticone(kf){
	VALEURRANGensemble=top.frames.Num0.frames.paramcarte.transXNbaires("return Xnombredaires")
	
for(i=1;i<VALEURRANGensemble;i++){
var strocWidth=top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("c"+i).getAttribute("stroke-width").replace("px","")	
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("c"+i).setAttribute("stroke-width",(strocWidth*kf)+"px")
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("n"+i).setAttribute("stroke-width",(strocWidth*kf)+"px")
	top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("f"+i).setAttribute("stroke-width",(strocWidth*kf)+"px")
}
}
...
top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("totalsvg").setAttribute("transform",mat)
top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("images1").setAttribute("transform",mat)
ajustetraiticone(1+zoomFactor/100)
ajusttraitetpolice()
...
top.frames.Num0.frames.couches.zoom.document.getElementById("trans").getSVGDocument().getElementById("c"+p).setAttribute("stroke-width",5.5)
}
}
function zoom0(){
...
if(!document.allxxxx){window.frames.zoom.scrollTo(trucx,trucy)};
