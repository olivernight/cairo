	...
	*/
	__showHideContentDiv : function(index){
		if(index!==false){	// Still content in this pane
			...
			if(indicclick==1){ 
			if(qsdf=='center'){
			if( index!=0){document.getElementById('centerContent').style.visibility="hidden";}else{document.getElementById('centerContent').style.visibility="visible";document.getElementById(htmlElementId).style.display='none';}
			}else{document.getElementById(htmlElementId).style.display='none';}
			}else{document.getElementById(htmlElementId).style.display='none';}
...
sizeCenterPane=Centxsize//Centxsize est définie dans A0 encaps.html
		...
		// Position east pane
		var posEastPane = posLeftCenterPane + sizeCenterPane//this.panesAssociative['center'].divElement.offsetWidth;
...
//(parseInt(document.getElementById("eastContent").parentNode.parentNode.style.width)-20)+"px";
...
DHTMLSuite.colorWidget.prototype = 
{
	...
	setRgbColor : function(rgbColor){
		...
		this.__changeViewAfterColorChange(false);
	}
	// }}}
	,
	// {{{ setHue()
	 ...
	 * @private
	 */
	__changeViewAfterColorChange : function(rgbColorChanged){
		this.__setCurrentRgbCode(rgbColorChanged);
		this.__setPaletteBgColor();
		this.__setBgColorPreviewDiv();
		this.__setSliderPos();
		this.__updateRgbInForm();
	...
	__setHueFromRgbColorsInForm : function(){
		...
		this.currentSaturation = Math.round(hsv.saturation*100);
		this.currentBrightness = Math.round(hsv.brightness*100);
		this.__changeViewAfterColorChange(true);
	}
	// }}}
	,
	...
	__receiveRgbCodeFromForm : function(e){
		...
		var rgbColors = this.colorHelper.getRgbColorsByRgbCode(rgbCode);
		this.fieldRed.value = rgbColors.red;
		this.fieldGreen.value = rgbColors.green;
		this.fieldBlue.value = rgbColors.blue;
	 ...
	 * @private
	 */
	__setCurrentRgbCode : function(rgbCodeChanged){
		if(!rgbCodeChanged) {
			...
			this.currentRgbCode = this.colorHelper.getRgbCodeByRgbColors(this.fieldRed.value,this.fieldGreen.value,this.fieldBlue.value);
...
DHTMLSuite.formValidator = function(propArray){
	...
	this.masks.letter = /^[A-Zï¿½ï¿½ï¿½]+$/gi;	// letteric
